...
public class FinalTutorController : MonoBehaviour
{
    [Header("Final Tutor Settings")]
    [SerializeField] private string tutorObjectName = "FinalTutor"; // Name to find the tutor GameObject
    [SerializeField] private int targetDay = 90; // Day when tutor should appear
    [SerializeField] private float startTime = 12f; // 12:00 PM
    [SerializeField] private float endTime = 15f; // 3:00 PM
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("FinalTutorController: Could not find DayTime controller through GameManager!");
        }
    ...
    private void CheckAndSetInitialTutorState()
    {
        ...
        if (shouldBeActive)
        {
            Debug.Log($"FinalTutorController: Final Tutor set active on scene load - Day {timeController.days + 1} at {timeController.GetTimeString()}");
        }
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (Application.isPlaying)
        {
            UnityEditor.Handles.Label(transform.position,
                $"Final Tutor Found: {HasTutorReference}\n" +
